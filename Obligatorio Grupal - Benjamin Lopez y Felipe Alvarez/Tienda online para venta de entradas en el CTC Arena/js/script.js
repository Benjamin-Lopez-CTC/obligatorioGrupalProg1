...
window.addEventListener('load', function() {
    ...
    if (eventosAlmacenados.length > 0) {
        cargarEventos(eventosAlmacenados);
    } else {
...
// Limpiar el carrito de la persona al hacer click en finalizar
document.querySelector('.checkout').addEventListener('click', function() {
    ...
    if (carrito.length > 0) {
        ...
        carrito.forEach(eventoComprado => {
            let evento = eventosLS.find(evento => evento.tipo === eventoComprado.tipo);
            if (evento) {
                evento.cantidadVendida +=1;
            }
        });
